 ...
 */
export async function seedBlogPostTemplate(db: PostgresJsDatabase): Promise<void> {
    ...
    if (existingTemplates.length === 0) {
        ...
        await db.update(globalContentTemplates)
...
export async function seedBlogPostContentType(
    ...
    if (existingContentType.length === 0) {
        console.log(`Creating content type '${BLOG_POST_CONTENT_TYPE_SLUG}' for workspace ${workspaceId}...`);
        ...
        console.log(`Content type '${BLOG_POST_CONTENT_TYPE_SLUG}' already exists for workspace ${workspaceId}. Skipping.`);
    }
}
 ...
 */
export async function runSeed(db: PostgresJsDatabase, workspaceId: string): Promise<void> {
