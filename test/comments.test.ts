...
describe.skipIf(process.env.RUN_INTEGRATION_TESTS !== "true")(
  ...
  () => {
    ...
    test("should query comments by workspaceId and entryId", async () => {
      ...
      expect(comments.every((c) => c.workspaceId === testWorkspaceId)).toBe(
        true,
      );
    ...
    test("should reject invalid entryId due to FK constraint", async () => {
      ...
      const databaseMessage =
        (error as (Error & { cause?: Error }) | null)?.cause?.message ??
        error?.message;
      expect(databaseMessage).toContain("violates foreign key constraint");
    });
  },
);
